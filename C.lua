--[[
    ...
    All internal API uses C. prefix (CelestialConfig v3.0.0 convention)
]]
...
if not Config or not Config.SV then
	...
	Config = {
		UIS = UISf, TS = TSf,
		PS = game:GetService("Players"), CG = game:GetService("CoreGui"),
		HS = game:GetService("HttpService"), RUN = game:GetService("RunService"),
		TXS = game:GetService("TextService"), SND = game:GetService("SoundService"),
		WS = game:GetService("Workspace"), LG = game:GetService("Lighting"),
		Safe = function(fn, ...)
		...
		TwF = TweenInfo.new(0.12, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
		TwM = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
		TwS = TweenInfo.new(0.38, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
		TwSP = TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
		TwSO = TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
		N = {
			Make = makef,
		...
		VERSION = "3.0.0",
		NAME = "Celestial",
		FOLDER_ROOT = "Celestial",
		FOLDER_CFG = "Celestial/Configs",
		CFG_EXT = ".cltl",
	}
end

-- Treat Config as C — all API is C. prefixed
local C = Config
...
------------------------------------------------------------------------
-- Thin alias layer over C (CelestialConfig v3.0.0 API)
-- Everything comes from C. — no Config.SV. direct access
------------------------------------------------------------------------

-- Services
local UIS           = C.UIS
local TweenService  = C.TS
local Players       = C.PS
local CoreGui       = C.CG
local Http          = C.HS
local Run           = C.RUN
local TextService   = C.TXS
local SoundService  = C.SND

-- Extra services
local Workspace_    = C.WS
local Lighting      = C.LG
local ReplicatedStorage = C.RS
local StarterGui    = C.SG
local NotificationService = C.NS
local VirtualInputManager = C.VIM
local GuiService    = C.GUI
local MarketplaceService = C.MS
local TeleportService = C.TP
local Teams_        = C.TM

-- Utilities
local safecall      = C.Safe
local folder        = C.File.MakeFolder
local tw            = C.Tween
local T_FAST        = C.TwF
local T_MED         = C.TwM
local T_SLOW        = C.TwS
local T_SPRING      = C.TwSP
local T_SPRING_SOFT = C.TwSO

-- Instance factory
local make          = C.N.Make
local corner        = C.N.Corner
local pad           = C.N.Pad
local stroke        = C.N.Stroke
local listLayout    = C.N.List
local gradient      = C.N.Gradient

-- Constants
local VERSION       = C.VERSION
local NAME          = C.NAME
local FOLDER_ROOT   = C.FOLDER_ROOT
local FOLDER_CFG    = C.FOLDER_CFG
local CFG_EXT       = C.CFG_EXT

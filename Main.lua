...
if not Config or not Config.SV then
	...
	local function makef(cls, props, parent)
		...
		for k, v in props do pcall(function() (i :: any)[k] = v end) end
		if parent then i.Parent = parent end
		return i
	end
	Config = {
		SV = { UserInputService = UISf, TweenService = TSf, Players = game:GetService("Players"), CoreGui = game:GetService("CoreGui"), HttpService = game:GetService("HttpService"), RunService = game:GetService("RunService"), TextService = game:GetService("TextService"), SoundService = game:GetService("SoundService") },
		Safe = function(fn, ...) if type(fn) ~= "function" then return nil end local ok, r = pcall(fn, ...) if not ok then warn("[Celestial] "..tostring(r)) return nil end return r end,
		Tween = function(inst, info, goals) local t = TSf:Create(inst, info, goals) t:Play() return t end,
		TW = {
...
-- ============================================================
-- Everything below is a thin alias layer over CelestialConfig.lua's
-- shared util module (C.SV / C.N / C.Tween / C.TW / C.Safe). This is what
-- keeps this file from re-defining make/corner/pad/stroke/tween-presets/
-- safecall on every project — one shared config, many UI scripts.
-- ============================================================
...
-- Emergency fallback if the config's theme table didn't come through —
-- keeps the library usable (single theme) rather than crashing outright.
if not Themes or not next(Themes) then
	warn("[Celestial] Config.Th missing — using a single built-in fallback theme.")
	Themes = {
		Nebula = {
...
if not next(Themes) then
	...
	Celestial.ActiveTheme = {
		...
		-- minimal fallback set to prevent crashes
	}
end

-- ============================================================
-- ICON SHORTHAND MAP — use Icon = "settings" instead of a raw asset id
-- ============================================================
Celestial.Icons = {
...
-- ============================================================
-- SOUND FEEDBACK — toggle globally with Celestial.SoundEnabled = false
-- ============================================================
...
-- ============================================================
-- GLOBAL TOOLTIP — attach a hover tooltip to any GuiObject
-- ============================================================
...
-- ============================================================
-- THEME REGISTRATION — add or override a theme at runtime
-- ============================================================
function Celestial:RegisterTheme(name, themeTable)
	local base = Themes.Nebula
	local merged = {}
...
function Celestial:Notify(opts)
	...
	-- Guard: if notification layer still doesn't exist, log to console instead
	if not _notifHolder then
		print("[Celestial Notify] " .. (opts.Title or "Notification") .. ": " .. (opts.Content or ""))
		return
	end
	
	-- Cap visible notifications so a burst of alerts can't flood the screen;
	-- oldest gets dismissed early to make room.
...
-- Shorthand: Celestial:Toast("Saved!", "success") instead of the full opts table
...
-- ============================================================
-- CONFIRM MODAL — blocking-style Yes/No dialog over a dim backdrop.
-- Callback receives `true` for confirm, `false` for cancel/dismiss.
-- ============================================================
function Celestial:Confirm(opts)
	...
	tw(dim, T_MED, { BackgroundTransparency = 0.45 }):Play()
	...
	local function finish(result)
		tw(dim, T_MED, { BackgroundTransparency = 1 }):Play()
		tw(box, T_MED, { BackgroundTransparency = 1 }):Play()
	...
	tw(box, T_SPRING_SOFT, { BackgroundTransparency = 0.05, Size = UDim2.new(0, 320, 0, 150), Position = UDim2.new(0.5, -160, 0.5, -75) }):Play()
	return screen
end
...
function Celestial:CreateWindow(opts)
	...
	-- Config + remembered window position/size (restored below, before the
	-- window is actually built, so it opens where you left it last time).
	...
	-- Robust parenting with fallback
	local parentSuccess = false
	if CoreGui then
		parentSuccess = pcall(function() screen.Parent = CoreGui end)
	end
	if not parentSuccess and game:GetService("CoreGui") then
	...
	do
		...
		topbar.InputEnded:Connect(function(i)
			if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
				if drag then
					-- Edge snapping: dropping the window within ~24px of a screen
					-- edge snaps it flush to that edge, Windows/Fluent style.
					local SNAP = 24
					...
					if snapped then
						tw(win, T_FAST, { Position = UDim2.new(pos.X.Scale, x, pos.Y.Scale, y) }):Play()
					end
				end
				drag = false
			end
	...
	-- Live search box — filters tab buttons by name as you type.
	-- Hidden automatically until 4+ tabs exist (added post-hoc below).
	...
	-- Resize grip (bottom-right corner). Sidebar/content are sized relative
	-- to `win`, so resizing just works without touching their layout code.
	...
	task.wait()
	local targetPos = if savedPos and savedPos.x and savedPos.y
		then UDim2.new(0, savedPos.x, 0, savedPos.y)
		else UDim2.new(0.5, -W/2, 0.5, -H/2)
	tw(win, T_SPRING, { Size = UDim2.new(0, W, 0, H), Position = targetPos, BackgroundTransparency = t.WindowTrans }):Play()
	...
	-- Accessibility: adjustable UI scale (0.8 = compact, 1.25 = large text/hitboxes)
	local uiScale = make("UIScale", { Scale = 1 }, win)
	function Window:SetScale(factor)
		factor = math.clamp(factor, 0.75, 1.5)
		tw(uiScale, T_MED, { Scale = factor }):Play()
	end
	...
	if cfgOpts and cfgOpts.Enabled then
		...
		-- Persist window position/size so it reopens where you left it.
	...
	function Window:CreateTab(tabOpts, parentOverride)
		...
		-- Thin visual separator between groups of controls
		...
		-- Plain static text, for short notes that don't need the full
		-- title+body paragraph card
		...
		-- Read-only progress indicator (0-1 or 0-100 depending on Max)
